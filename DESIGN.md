...
- Two accents with different jobs: periwinkle `#B9B8EF` is where an interactive element *arrives*, indigo `#4845DA` means *live* — the status dot, the needle, the wash under a panel you are about to open.
...
- **Section numbers on the index.** Two tools in a fixed order carry no information in `01` / `02`, so the numbers went. Sequence numbers remain only where they are data: the layer stack, the queue, the row counter.
- **Eyebrow lines above headings.** A tool panel's tagline sits *under* its rule, at the bottom of the panel, beside the affordance — never above the name.
...
**The Two-Job Rule.** Periwinkle is where you *arrive*; indigo means something is *live*. An element that cannot be acted on never sits in periwinkle, and an element that is merely hovered never turns indigo.
...
### Hierarchy
- **Display** (700, 24px, `0.02em`, upper): the lead tool's name on the index. One per page at most.
...
**Three measures.** `max-w-3xl` for reading, `max-w-6xl` for squoosh, `max-w-[1600px]` for the merge editor. A tool takes the narrowest measure that fits its work.
...
**Borders** are 1px of ink at 12–16% alpha. A dashed border means one thing only: *this region is waiting for a file*.
...
### Tool panel (signature component)
- **Composition:** name, blurb, then a hairline, then the tagline and `OPEN →` on one row beneath it. The tagline is below the rule on purpose — above the heading it would be an eyebrow.
- **Grid:** the lead tool takes 3 of 5 columns and display type; the rest take 2 and headline type. Two identical cards would claim the tools are interchangeable.
- **Hover:** the border moves to periwinkle at 45%, the fill lifts from 3.5% to 6%, the name turns periwinkle, the arrow translates 4px, and a 128px indigo wash rises from the floor of the panel over 500ms.
...
- **Do** say plainly when the server is unreachable, and keep both tools working without it.
